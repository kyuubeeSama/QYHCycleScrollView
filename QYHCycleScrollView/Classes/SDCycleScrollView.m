...
- (void)scrollToIndex:(int)targetIndex
{
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            [self scrollViewBeginScrollingAnimation:self.mainView];
...
-(void)scrollViewBeginScrollingAnimation:(UIScrollView *)scrollView{
    if (!self.imagePathsGroup.count) return; // 解决清除timer时偶尔会出现的问题
    int itemIndex = [self currentIndex];
    itemIndex += 1;
    int indexOnPageControl = [self pageControlIndexWithCurrentCellIndex:itemIndex];
    if (self.itemBeginScrollOperationBlock) {
        self.itemBeginScrollOperationBlock(indexOnPageControl);
    }
}
